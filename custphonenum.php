...
<html lang="en">
  ...
  <!-- get -->
	...
	$sql = 'SELECT * FROM custphonenum;';
	$result = $conn->query($sql);
	
	$list = array(array("Customer Phone Number"=>"","Customer ID"=>""));
	if ($result->num_rows > 0) {
		// output data of each row
		while($row = $result->fetch_assoc()) {
			array_push($list, array($row["CustomerPhoneNumber"],$row["CustID"]));
		}
	} else {
		echo "0 results";
	}
	...
	<?php endif; ?>
		...
		<a class="waves-effect waves-light btn-large modal-trigger" data-target="modify" href="custphonenum_modify.php"><i class="material-icons left">swap_vertical_circle</i>MODIFY</a>
