...
<html lang="en">
  ...
  <!-- get -->
	...
	$list = array(array("ShipmentID"=>"","DistributorID"=>"","StoreID"=>""));
	if ($result->num_rows > 0) {
		// output data of each row
		while($row = $result->fetch_assoc()) {
			array_push($list, array($row["ShipmentID"],$row["DistributorID"],$row["StoreID"]));
		}
	} else {
		echo "0 results";
	}
	...
	<?php endif; ?>
		...
		<a class="waves-effect waves-light btn-large modal-trigger" data-target="modify" href="shipment_modify.php"><i class="material-icons left">swap_vertical_circle</i>MODIFY</a>
